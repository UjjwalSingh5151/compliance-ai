...
function ScoreRing({ pct }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: 90, height: 90, borderRadius: "50%", border: `4px solid ${color}`, background: `${color}15`, flexShrink: 0 }}>
      <span style={{ fontSize: 22, fontWeight: 700, color }}>{pct}%</span>
    </div>
  );
}

function QuestionCard({ q, comment, onCommentChange, onCommentSave, saving }) {
  ...
  return (
    <div style={{ border: `1px solid ${c.border}`, borderRadius: 10, overflow: "hidden", marginBottom: 10 }}>
      ...
      <div
        style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", cursor: "pointer", background: c.card }}
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          ...
          {comment && <span style={{ fontSize: 10, color: c.purple, background: c.purpleDim, padding: "2px 6px", borderRadius: 4 }}>commented</span>}
      ...
      {/* Expanded detail */}
      {expanded && (
        <div style={{ padding: 16, background: c.bg, borderTop: `1px solid ${c.border}`, display: "flex", flexDirection: "column", gap: 12 }}>

          {/* Student's answer */}
          {q.student_answer && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: c.textDim, marginBottom: 4, letterSpacing: 0.5 }}>STUDENT'S ANSWER</div>
              <div style={{ fontSize: 13, color: c.textMid, lineHeight: 1.65, background: c.card, padding: "10px 14px", borderRadius: 8, border: `1px solid ${c.border}` }}>
                {q.student_answer}
              </div>
            </div>
          )}
          ...
          {q.reasoning && (
            <div>
              ...
              <div style={{ fontSize: 13, color: c.textMid, lineHeight: 1.65, background: c.accentDim, padding: "10px 14px", borderRadius: 8, border: `1px solid ${c.accent}30` }}>
                {q.reasoning}
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Add your comment or override note for this question…"
              rows={2}
...
export default function ResultDetail({ params, navigate }) {
  ...
  if (error || !result) return <div style={{ padding: 40, color: c.danger }}>Error: {error || "Not found"}</div>;
  ...
  return (
    <div style={{ padding: 28, maxWidth: 820, margin: "0 auto" }}>
      ...
      <div style={{ ...card, display: "flex", gap: 20, alignItems: "center", marginBottom: 16 }}>
        <ScoreRing pct={pct} />
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: c.text }}>{student?.name || "Unknown Student"}</div>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: c.text, marginTop: 6 }}>
            {result.marks_obtained} <span style={{ fontSize: 14, fontWeight: 400, color: c.textMid }}>/ {result.total_marks} marks</span>
          </div>
        </div>
      ...
      {analysis?.questions?.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: c.textMid, letterSpacing: 0.5 }}>
              QUESTION-WISE BREAKDOWN — click any question to expand
            </div>
            <button style={{ ...btn.ghost, fontSize: 12, color: c.purple, border: `1px solid ${c.purple}40`, borderRadius: 6, padding: "5px 14px", opacity: saving ? 0.6 : 1 }}
